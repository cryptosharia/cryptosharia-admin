<script lang="ts">
	...
	let stats = $derived([
		{ label: 'Total Tokens', value: data.tokenCount.toLocaleString(), change: '+20.1% from last month', icon: TrendingUp, color: 'text-orange-500' },
		{ label: 'Administrators', value: data.adminCount.toLocaleString(), change: '+2 since last hour', icon: ShieldCheck, color: 'text-emerald-500' },
		{ label: 'Contributors', value: data.contributorCount.toLocaleString(), change: '+10 new contributors', icon: Users, color: 'text-blue-500' },
		{ label: 'Messages', value: data.messageCount.toLocaleString(), change: '+19 unread messages', icon: Bell, color: 'text-purple-500' }
	]);
</script>
