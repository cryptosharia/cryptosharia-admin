<script lang="ts">
	...
	// Dynamic Area Keahlian (Expertise)
...
<div class="max-w-4xl mx-auto space-y-6 pb-12">
	...
	<div class="flex items-center justify-between">
		<div class="flex items-center gap-3">
			...
			<div>
				<h1 class="text-2xl font-bold tracking-tight text-foreground">Tambah Anggota Tim</h1>
				<p class="text-sm text-muted-foreground">Tambahkan profil baru untuk ditampilkan di Kenali Tim CryptoSharia</p>
			</div>
		</div>
	</div>
		...
		use:enhance={() => {
			...
			return async ({ result, update }) => {
				...
				if (result.type === 'failure') {
					toast.error((result.data as any)?.message || 'Gagal menyimpan data.');
				}
				await update();
			};
		}}
	...
	>
		...
		<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
			<!-- Foto Profil & Status (Sidebar Kolom Kiri) -->
			<div class="space-y-6">
