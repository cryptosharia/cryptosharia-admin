...
<div class="space-y-6">
	...
	<div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
		<div>
			...
			<p class="mt-1 text-sm text-muted-foreground sm:text-base">Manage your crypto tokens here.</p>
		</div>
						...
						{#each data.tokens || [] as token}
							<Table.Row class="group">
									...
									<div class="flex items-center gap-3 font-medium">
										{#if token.logoUrl}
											<img
												src={token.logoUrl}
												alt={token.name}
												class="h-8 w-8 rounded-full bg-secondary"
											/>
										{:else}
											<div
												class="flex h-8 w-8 items-center justify-center rounded-full bg-secondary text-xs font-bold text-muted-foreground"
											>
												{token.ticker?.[0] || '?'}
											</div>
										{/if}
								...
								<Table.Cell>
									<Badge
										variant="outline"
										class="capitalize {token.shariaStatus === 'halal'
											? 'border-emerald-500/20 bg-emerald-500/10 text-emerald-500'
											: token.shariaStatus === 'haram'
								...
								<Table.Cell>
									<span class="text-sm text-muted-foreground capitalize">{token.status}</span>
								</Table.Cell>
									...
									>
										<svg
											...
											><path d="M12 20h9"></path><path
												d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4 Z"
											></path></svg
										>
									</Button>
								</Table.Cell>
