...
<div class="max-w-2xl mx-auto space-y-6">
	<div class="flex items-center justify-between">
		<div class="flex items-center gap-4">
			...
			<div>
				<h1 class="text-3xl font-bold tracking-tight text-foreground">Edit Principle</h1>
				<p class="text-muted-foreground">Update the Sharia evaluation principle.</p>
			</div>
		</div>
		
		<form action="?/delete" method="POST" use:enhance onsubmit={(e) => !confirm('Are you sure you want to delete this principle?') && e.preventDefault()}>
				...
				<div class="grid grid-cols-1 gap-6">
					...
					<div class="space-y-2">
						...
						<Textarea
							...
							rows={4}
							class="resize-none"
						>{data.principle.description}</Textarea>
					</div>
					...
					<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
						...
						<div class="space-y-2">
							...
							<Input
								...
								id="displayOrder"
								name="displayOrder"
								value={data.principle.displayOrder}
							/>
						</div>
					</div>
					...
					<div class="space-y-2">
						...
						>
							{#each data.icons as icon}
								<option value={icon.id} selected={icon.id === data.principle.iconId}>{icon.icon} ({icon.type})</option>
							{/each}
						</select>
					</div>
				</div>
