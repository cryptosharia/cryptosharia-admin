...
<div class="max-w-2xl mx-auto space-y-6">
	<div class="flex items-center justify-between">
		<div class="flex items-center gap-4">
			<Button href="/admins" variant="outline" size="icon" class="rounded-full">
				<ArrowLeft size={18} />
			</Button>
			<div>
				<h1 class="text-3xl font-bold tracking-tight text-foreground">Edit Admin</h1>
				<p class="text-muted-foreground">Update system administrator account.</p>
			</div>
		</div>
		
		<form action="?/delete" method="POST" use:enhance onsubmit={(e) => !confirm('Are you sure you want to delete this admin?') && e.preventDefault()}>
				...
				<div class="grid grid-cols-1 gap-6">
					...
					<div class="space-y-2">
						...
						>
							{#each data.roles as role}
								<option value={role.id} selected={role.id === data.admin.roleId}>{role.name}</option>
							{/each}
						</select>
					</div>
					...
					<div class="flex items-center space-x-2">
						<input
							...
							id="isActive"
							name="isActive"
							checked={data.admin.status === 'active'}
							class="h-4 w-4 rounded border-input text-primary focus:ring-ring"
						/>
						<label for="isActive" class="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">Account Active</label>
					</div>
				</div>

				<div class="flex justify-end gap-3 pt-6 border-t">
					<Button href="/admins" variant="ghost">Cancel</Button>
